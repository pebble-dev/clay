 ...
 * @returns {void}
 */
function openPicker(colorItem: ClayItemInstance): void {
...
describe('component - color', function() {
  ...
  it('shows the picker when the label is clicked', function() {
    ...
    const $picker = colorItem.$element.select('.picker-wrap');
    assert.strictEqual($picker[0].classList.contains('show'), false);
    openPicker(colorItem);
    assert.strictEqual($picker[0].classList.contains('show'), true);
  });
